...
void CreditBasedGate::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        // added by ph
        if (strcmp(par("creditMode").stringValue(), "standard") == 0)
        {
            creditMode = standard;
            accumulateCreditInGuardBand = true;
            // set gbMode = worstCase in periodicGate
        }
        else if (strcmp(par("creditMode").stringValue(), "frozen") == 0)
        {
            creditMode = frozen;
            accumulateCreditInGuardBand = false;
            // set gbMode = worstCase in periodicGate
        }
        else if (strcmp(par("creditMode").stringValue(), "returnToZero") == 0)
        {
            creditMode = returnToZero;
            accumulateCreditInGuardBand = false;
            // set gbMode = bestCase in periodicGate
        }
        else
        {
            throw cRuntimeError("Parameter creditMode of %s is %s and is only allowed to be standard, frozen or returnToZero", getFullPath().c_str(),
                    par("creditMode").stringValue());
        }
        ...
        if (periodicGate != nullptr) {
            ...
            if (!(creditMode == standard) || !accumulateCreditInGuardBand) // only subscribe signal, when creditMode is not standard
...
void CreditBasedGate::scheduleChangeTimer()
{
    ...
    /* CreditBasedGate: Changed scheduleChangeTimer() to cancel the timer event if the transmitCreditLimit is unreachable.
    ...
    if (currentCreditGainRate == 0 ||
        ...
        if (changeTime < now)
            cancelEvent(changeTimer); // cancelled also when credit felt with sendslope
        else
...
void CreditBasedGate::updateCurrentCreditGainRate()
{
    ...
    else if (periodicGate != nullptr && (periodicGate->isClosed() || (((creditMode == frozen || (creditMode == returnToZero && currentCredit >= 0)) && periodicGate->isInGuardBand())))){
