...
simsignal_t PacketGateBase::gateTransmissionAllowedStateChangedSignal = registerSignal("gateTransmissionAllowedStateChanged"); // added by ph
...
void PacketGateBase::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        emit(gateTransmissionAllowedStateChangedSignal, isOpen_); // added by ph
}
...
void PacketGateBase::open()
{
    ...
    isOpen_ = true;
    // emit(gateTransmissionAllowedStateChangedSignal, isOpen_); // sets transmissionAllowed_ to true in preemptable Streamer // added by ph
    if (producer != nullptr)
        producer->handleCanPushPacketChanged(inputGate->getPathStartGate());
    if (collector != nullptr)
        collector->handleCanPullPacketChanged(outputGate->getPathEndGate());
    // emit(gateTransmissionAllowedStateChangedSignal, isOpen_); // sets transmissionAllowed_ to true in preemptable Streamer // added by ph
    emit(gateStateChangedSignal, isOpen_); // old
    updateDisplayString();
}
