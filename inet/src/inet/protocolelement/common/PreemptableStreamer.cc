...
void PreemptableStreamer::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        WATCH(isTransmissionAllowed_);
        WATCH(isGuardbandActive);
        WATCH(isGateOpen);
        WATCH(modOn);

        cModule *module = this->getParentModule()->getSubmodule("queue")->getSubmodule("transmissionGate",0);
        if (module != nullptr){
            module->subscribe(PeriodicGate::gateTransmissionAllowedStateChangedSignal, this);
        }
...
// added by ph
// we only have to tell the preemptable streamer
// that it must not stream the remainingPacket, when the associated periodicGate is closed
void PreemptableStreamer::receiveSignal(cComponent *source, simsignal_t signal, bool value, cObject *details)
{
    Enter_Method("%s", cComponent::getSignalName(signal));
    if (signal == PeriodicGate::gateTransmissionAllowedStateChangedSignal) {
        EV_INFO << "Processing received gate state changed signal" << EV_ENDL;
        isTransmissionAllowed_ = value;
    }
    else
...
{
    return !isStreaming() && isTransmissionAllowed_ && (remainingPacket != nullptr || provider->canPullSomePacket(inputGate->getPathStartGate()));
}

Packet *PreemptableStreamer::canPullPacket(cGate *gate) const
{
    return (isStreaming() || !isTransmissionAllowed_)  ? nullptr : remainingPacket != nullptr ? remainingPacket : provider->canPullPacket(inputGate->getPathStartGate());
}
